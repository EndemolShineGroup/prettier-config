...
- **Node 8 LTS:** `prettier-config` has a minimum Node version
...
``` bash
$ git clone https://github.com/EndemolShineGroup/prettier-config.git
$ cd prettier-config
$ yarn
$ yarn test
```
...
We want to ensure that `prettier-config` is secure for everyone. If
